...
- `GET /ingredients/:id/ai-translations` - superadmin only; lists cached AI-generated translations (`ai_content_translations` rows) for the ingredient, across all locales/fields. See the `internationalization` goal's `ai-auto-translation` feature.
...
- `GET /.../dishes/:dishId/ai-translations` - superadmin only; lists cached AI-generated translations for the dish. See the `internationalization` goal's `ai-auto-translation` feature.
...
- `GET /public/restaurants/:slug/menu` - published menus only, active restaurants only. Nested menus → sections → dishes (with `media`) → ingredients (with `media`; `imageUrl` derived from first gallery image). Approved ingredients globally, or pending when `requested_by_restaurant_id` matches. Shape: `publicMenuResponseSchema` in `@digital-menu/shared`. Accepts `?locale=` (BCP-47, must be on the fixed allow-list in `SUPPORTED_TRANSLATION_LOCALES` / `TRANSLATION_LOCALES` env override - anything else is silently treated as no locale). When present and allow-listed, dish/ingredient name/description/allergen-group are resolved per-field: an existing manual translation (`dish_translations`/`ingredient_translations`) always wins; otherwise the AI cache (`ai_content_translations`) is checked by source-text hash, regenerating via Gemini on a miss with a ~2.5s timeout and falling back to the source-language column on any failure. No diner-facing indicator of which path served the response. See `docs/goals/internationalization/features/ai-auto-translation/`.
...
- `/r/[slug]` - server-rendered public menu; ingredient links open `?i=<slug>` modal; logged-in users see restriction badges. `?locale=` selects the AI/manual-translated language (see `internationalization` goal); absent an explicit param, `Accept-Language` only pre-selects the `LanguagePicker`'s default, never a silent override.
...
14. **AI auto-translation**: with `GEMINI_API_KEY` set, use the `LanguagePicker` on `/r/<slug>` to switch to a supported language (e.g. Vietnamese); confirm dish name/description translate (first switch pays LLM latency, second is instant from cache - check `ai_content_translations` gets a row). In the admin portal as superadmin, open a dish's/ingredient's Translations panel and confirm the "AI-generated translations" section lists it; log in as a restaurant admin instead and confirm that section is absent. Save a manual translation for the same dish/locale via the existing panel, reload the diner page in that locale, and confirm the manual text now wins. Try `?locale=xx-not-real` directly against the API - confirm it silently falls back to English rather than erroring or calling the model.
