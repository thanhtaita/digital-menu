-- Smoke-test queries for the fdc schema, shaped like the API calls
-- apps/api would actually make once ingredients.fdc_id starts getting
-- populated. Run any of these directly in psql/a DB GUI, or via
-- `python smoke_test.py` in this directory to see them all execute.

-- 1. Ingredient search-as-you-type (e.g. GET /ingredients/search?q=tomato)
SELECT fdc_id, description, data_type
FROM fdc.food
WHERE description ILIKE '%egg white%'
...
-- haven't silently grown (would indicate a bad reload)
SELECT
