...
- **Translations**: `dish_translations`, `ingredient_translations` - locale-keyed human-entered overlays, separate from `ingredient_aliases`. `ai_content_translations` - polymorphic (`entityType`/`entityId`, `dish`|`ingredient` only), hash-keyed (`sourceHash`) disposable cache for machine-generated translations; a human row in the two tables above always takes precedence and is never consulted alongside this table - see [`docs/goals/internationalization/features/ai-auto-translation/`](../goals/internationalization/features/ai-auto-translation/README.md).
